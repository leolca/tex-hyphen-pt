...
orthographers \cite{gandavo1574}, few documents used such sign until the end of
the 18th century \cite{araujo2015}.
...
\cite{kristensen2001}. The study revealed that, overall, the \TeX{} approach
...
\cite{knuth1977}, primarily focused on the English language and employed three
main rules:
...
\cite{liang1983}.  The \TeX{} hyphenation algorithm later adopted the approach
...
Liang \cite{liangbreitenlohner1999} and has been utilized to generate
...
If we consider only the Latin alphabet, with no diacritcs, the patterns used 
in \TeX{} hyphenation are of the form: \texttt{\^{}\textbackslash{}.?[0-9]?([a-zA-Z]+[0-9]?)+\textbackslash{}.?\$},
where we have described it using a regular expression\footnote{
    ...
    \texttt{([a-zA-Z]+[0-9]?)+} matches sequences of letters (both uppercase and lowercase) interleaved with optional digits.
}. 
One example of such pattern is \texttt{4z1z2}. Each
